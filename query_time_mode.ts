...
import { QueryInput, QueryOutput, QueryOutput_time_mode } from './api'
...
export function query(input: QueryInput) {
  ...
  // time slots for
  const time_slots = getTimeIdsInRangeByTimeMode(times_ids, input.time_mode)
  ...
  function toQueryOutput(): QueryOutput_time_mode {
    const events: QueryOutput_time_mode['rainfall_events'] = []
      ...
      for (let month in counters[district_id]) {
        for (let day in counters[district_id][month]) {
          for (let time_index in counters[district_id][month][day]) {
            ...
            // filter (porxy.time, {id = time_id}) -> get start time hour and minute
...
export function addMinutesToTime(
  ...
  // Handle negative minutes (if subtracting)
  if (totalMinutes < 0) {
...
// year = view_year
// return start and end date in string format
// YYYY-MM-DD HH:MM
function getEventRange(
  ...
  const start_time = `${hour.toString().padStart(2, '0')}:${minute
    .toString()
    .padStart(2, '0')}`
  const start = `${year}:${month.toString().padStart(2, '0')}:${day
    .toString()
  ...
  switch (time_mode) {
    case '15mins':
      end_time = addMinutesToTime(start_time, 15)
      break
    case '2hrs':
      end_time = addMinutesToTime(start_time, 120)
      break
    case '12hrs':
      end_time = addMinutesToTime(start_time, 720)
      break
    case '24hrs':
      return { start, end: '' }
    default:
  ...
  const end = `${year}:${(date?.getMonth() + 1)
    .toString()
    .padStart(2, '0')}:${date
    ?.getDate()
    .toString()
...
function getTimeIdsInRangeByTimeMode(
  time_ids: number[],
  time_mode: '15mins' | '2hrs' | '12hrs' | '24hrs',
): TimeSlot[] {
